    ...
    <>
      ...
      <main className="flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="w-full lg:w-[80%] p-4 bg-gray-100">
          <h1 className="text-center mb-4 font-mono text-2xl md:text-4xl">Members Dashboard</h1>
            ...
            {isLodding ? (
              ...
              <>
                  ...
                  <table className="min-w-full text-left border-t border-gray-200 text-sm md:text-base">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        <th className="px-4 py-2 font-medium text-gray-600">Job Title</th>
                    ...
                    <tbody>
                      {members.map((member) => (
                        <tr key={member.id} className="border-t hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2">{member.job_title}</td>
                          <td className="px-4 py-2">{member.status ? 'Active' : 'Block'}</td>
                          <td className="px-4 py-2 flex flex-wrap gap-2 justify-center">
                            <Link to = {`/admin/members/edit/${member.id}`} className="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
                              Edit
                            </Link>
                            <button onClick={()=>deleteMember(member.id)} className="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">
                              Delete
                            </button>
                          </td>
                        </tr>
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                  {members.map((member) => (
                    <div key={member.id} className="border rounded-lg shadow-sm p-4 bg-gray-50">
                      <p><span className="font-semibold">ID:</span> {member.id}</p>
                      <p><span className="font-semibold">Name:</span> {member.name}</p>
                      <p><span className="font-semibold">Job Title:</span> {member.job_title}</p>
                      <p>
                        <span className="font-semibold">Status:</span>{' '}
                        {member.status ? 'Active' : 'Block'}
                      </p>
